...
class Path:
    ...
    def move_metro(self, metro: Metro, dt_ms: int) -> None:
        ...
        # Safety check: if distance is very small or zero, skip movement calculation
        if dist < 1e-6:  # Very small threshold
            metro.current_station = dst_station
            # Don't change direction vector, use current metro orientation
        ...
        if dist > travel_dist_in_dt and dist >= 1e-6:
